                                        ...
                                        uint64_t *previous, uintptr_t ra)
{
    ...
    flags = probe_access_full(env, addr, 8, MMU_DATA_STORE, mmu_idx, false,
                              &host, &full, ra);
    section = iotlb_to_section(env_cpu(env),
                               full->xlat_section & ~TARGET_PAGE_MASK,
                               full->attrs);
    writable_ram = memory_region_is_ram(section->mr) &&
    ...
    if (writable_ram) {
        if (host == NULL) {
             ...
             * pointer.  xlat_section still names the same RAM backing and is
             * safe to consume before the next TLB access.
             */
            host = qemu_map_ram_ptr(NULL, addr + full->xlat_section);
        }
